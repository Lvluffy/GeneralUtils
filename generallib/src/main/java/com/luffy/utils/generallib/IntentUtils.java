...
public class IntentUtils {
     ...
     * @param intentExtra 自定义传参
     */
    public void startActivityForResult(Context context, Class<?> cls, int requestCode, IntentExtra intentExtra) {
        if (context instanceof Activity) {
            Intent intent = new Intent(context, cls);
            if (intentExtra != null) {
                ((Activity) context).startActivityForResult(intentExtra.putExtra(intent), requestCode);
            } else {
                ((Activity) context).startActivityForResult(intent, requestCode);
            }
        } else {
            this.startActivity(context, cls, intentExtra);
        }
    }
